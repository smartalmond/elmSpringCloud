...
onMounted(() => {
  ...
  axios.post('BusinessController/getBusinessById', qs.stringify({ businessId: businessId.value }))
      .then(response => {
  ...
  axios.post('FoodController/listFoodByBusinessId', qs.stringify({ businessId: businessId.value }))
      .then(response => {
...
const saveCart = (index) => {
  axios.post('CartController/saveCart', qs.stringify({
    ...
    if (response.data === 1) {
      ...
      // 对 state.foodArr 进行排序（如果需要）
      foodArr.value.sort()
      // state.foodArr.sort((a, b) => /* 排序逻辑 */);
    } else {
      alert('向购物车中添加食品失败！');
    }
  }).catch(error => {
...
const updateCart = (index, num) => {
  axios.post('CartController/updateCart', qs.stringify({
    businessId: businessId.value,
    userId: user.value.userId, // 假设 userId 不是 ref
    foodId: foodArr.value[index].foodId,
